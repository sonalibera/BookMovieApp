...
//method allows us to execute the React code synchronously when the component gets loaded or mounted in the DOM (Document Object Model).
    componentWillMount() {
        let at = this;
        let dataMovie = null;
        let abcMovie = new XMLHttpRequest();
        abcMovie.addEventListener("readystatechange", function () {
        ...
        abcMovie.open("GET", this.props.baseUrl + "movies/" + this.props.match.params.id);
        abcMovie.setRequestHeader("Cache-Control", "no-cache");
        abcMovie.send(dataMovie);
    }
